      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="text-center mb-14">
          ...
          <h2 className="text-3xl sm:text-4xl font-black text-[var(--section-text)] mb-4">
            Powered by{" "}
            <span className="bg-gradient-to-r from-sky-500 to-blue-500 bg-clip-text text-transparent">World-Class Technology</span>
          </h2>
        ...
        <div className="relative overflow-hidden py-2 mb-14">
          ...
          <div className="animate-marquee flex gap-4 w-max">
            {doubled.map((tech, i) => (
              <motion.div key={i} whileHover={{ y: -6, scale: 1.05 }}
                className="group relative rounded-2xl px-6 py-4 flex flex-col items-center gap-1 shrink-0 min-w-[140px] backdrop-blur-sm shadow-md transition-all duration-300 bg-[var(--section-card-bg)] border border-[var(--section-card-border)] hover:shadow-[0_15px_35px_rgba(56,189,248,0.2)] hover:border-sky-400/30">
                ...
                <span className="text-sm font-bold text-[var(--section-text)] whitespace-nowrap">{tech.name}</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {columns.map((col, idx) => {
            const Icon = col.icon;
            return (
              <motion.div key={col.title} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: idx * 0.12 }} whileHover={{ y: -8 }}
                ...
                {/* Card Inner — token glass */}
                <div className="relative rounded-3xl p-7 h-full transition-colors duration-300 bg-[var(--section-card-bg)] backdrop-blur-sm border border-[var(--section-card-border)] group-hover:border-sky-400/20">
                  ...
                  <h4 className="text-sm font-black text-[var(--section-text)] uppercase tracking-widest mb-4">{col.title}</h4>

                  <ul className="space-y-3">
                    {col.items.map((item, i) => (
                      <motion.li key={item} initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: idx * 0.1 + i * 0.05 }}
                        className="flex items-center gap-2.5 text-sm transition-colors text-[var(--section-text-muted)] group-hover:text-[var(--section-text)]">
