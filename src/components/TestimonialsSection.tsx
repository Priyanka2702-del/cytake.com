      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-white/5 border border-white/15 backdrop-blur-sm px-4 py-1.5 rounded-full mb-4">
            ...
            <span className="text-xs font-bold text-sky-400 tracking-wide uppercase">Client Stories</span>
          </div>
        ...
        <div className="relative max-w-3xl mx-auto">
          <div className="overflow-hidden rounded-3xl">
            <AnimatePresence mode="wait" custom={dir}>
              <motion.div
                ...
                className="relative rounded-3xl p-8 sm:p-10 bg-white/[0.04] backdrop-blur-xl border border-white/10 shadow-[0_20px_60px_rgba(0,0,0,0.4)] overflow-hidden"
              >
                {/* Big Quote Background */}
                <Quote size={120} className="absolute -top-4 -right-4 text-white/5" />

                {/* Top Gradient Bar */}
                <div className="absolute top-0 left-0 right-0 h-1.5 bg-gradient-to-r from-blue-600 via-sky-500 to-indigo-500" />

                <div className="relative z-10">
                  <Quote size={36} className="text-sky-400 mb-6" />
                  ...
                  <div className="flex items-center justify-between flex-wrap gap-4 pt-6 border-t border-white/10">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-sky-400 font-semibold">{testimonials[current].country}</div>
                      </div>
                    </div>
          ...
          <div className="flex items-center justify-center gap-4 mt-8">
            <button
              onClick={prev}
              className="w-11 h-11 rounded-full bg-white/5 border border-white/15 flex items-center justify-center text-slate-400 hover:text-sky-400 hover:border-sky-400/50 transition-all"
            >
              <ChevronLeft size={18} />
            </button>
        ...
        >
          <p className="text-xs text-slate-500 uppercase tracking-widest mb-6 font-bold">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            {["United Kingdom", "UAE", "India", "Australia", "Singapore", "Cyprus", "Mauritius", "South Africa", "Hong Kong", "Cayman Islands"].map((c, i) => (
              <motion.span
                ...
                className="text-xs text-slate-300 font-semibold px-4 py-2 rounded-xl bg-white/[0.04] backdrop-blur-sm border border-white/10 hover:border-sky-400/30 hover:text-white transition-all cursor-default"
              >
                {c}
              </motion.span>
            ))}
