...
function IndustryCard({ industry, index }: { industry: any; index: number }) {
  ...
  const rotateX = useSpring(useTransform(y, [-40, 40], [10, -10]), {
    stiffness: 280,
    damping: 25,
  });
  ...
  return (
    ...
    >
      ...
      <div className="relative h-44 overflow-hidden">
        ...
        {/* Floating Orbs */}
        <motion.div
          className="absolute top-[-20px] right-[-20px] w-32 h-32 rounded-full bg-white/20 blur-2xl"
        ...
        >
          <motion.polyline
            points="0,60 50,45 100,55 150,30 200,40 250,20 300,35 350,15 400,25"
            fill="none"
            stroke="white"
            strokeWidth="2"
            strokeLinecap="round"
        ...
        <div
          className="absolute inset-0 opacity-0 group-hover:opacity-100 transition-opacity duration-700 bg-[linear-gradient(120deg,transparent,rgba(255,255,255,0.3),transparent)] -translate-x-full group-hover:translate-x-full"
        ...
        <motion.div
          ...
          style={{
            background: useTransform(
              [glowX, glowY],
              ([gx, gy]) =>
                `radial-gradient(circle at ${gx}% ${gy}%, rgba(56,189,248,0.1), transparent 60%)`
            ),
          }}
        />
...
export default function IndustriesSection() {
  return (
    <section className="py-24 relative overflow-hidden bg-gradient-to-br from-[#020617] via-[#0A1628] to-[#020617]">
      ...
      {/* Base Dark Gradient */}
      <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,#0B1B3F_0%,#020617_60%)]" />
          ...
          {[
            ...
            <g key={i}>
              ...
              <rect
                ...
                y={Math.min(c.o, c.c)}
                width="12"
                height={Math.max(4, Math.abs(c.o - c.c))}
                fill="#22C55E"
                opacity="0.4"
                rx="1.5"
              />
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        >
          <Link
            href="/industries"
            className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl text-sm font-bold text-white bg-gradient-to-r from-blue-600 to-sky-500 hover:shadow-[0_8px_25px_rgba(56,189,248,0.35)] hover:-translate-y-0.5 transition-all"
          >
