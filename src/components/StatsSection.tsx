...
const statsData = [
  {
    value: 200, suffix: "+", label: "Brokers Powered",
    desc: "Active forex and prop firm clients",
    icon: TrendingUp,
  ...
  },
  {
    ...
    desc: "Global reach across all continents",
    icon: Globe,
  ...
  },
  {
    ...
    desc: "Enterprise reliability guarantee",
    icon: Shield,
    color: "from-emerald-500 to-teal-400",
    glow: "rgba(16,185,129,0.4)",
  },
  {
    value: 5, suffix: "B+", label: "Volume Processed",
    desc: "USD in trading volume supported",
    icon: Zap,
  ...
  },
  {
    ...
    desc: "Proven forex technology expertise",
    icon: Award,
            ...
            return (
              <motion.div
                ...
                whileHover={{
                  y: -10,
                  scale: 1.04,
                  boxShadow: `0 30px 60px ${stat.glow}`,
                }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: i * 0.08 }}
                className="relative group bg-white/[0.04] backdrop-blur-sm border border-white/10 rounded-2xl p-5 text-center overflow-hidden cursor-default shadow-lg hover:border-white/20 transition-colors"
              >
                {/* Top gradient bar */}
                ...
                {/* Counter */}
                <div className="relative z-10 text-3xl sm:text-4xl font-black text-white mb-1">
                  <Counter target={stat.value} suffix={stat.suffix} decimals={stat.value % 1 !== 0 ? 1 : 0} />
                </div>

                {/* Label */}
                <div className="relative z-10 text-xs font-black text-slate-200 mt-1 tracking-wide">
                  {stat.label}
                </div>

                {/* Desc */}
                <div className="relative z-10 text-[10px] text-slate-400 mt-1 leading-tight">
                  {stat.desc}
                </div>

                {/* Bottom animated border */}
                <motion.div
                  className={`absolute bottom-0 left-0 h-0.5 bg-gradient-to-r ${stat.color}`}
                  initial={{ width: "0%" }}
        ...
        >
          ...
          <div className="relative z-10">
            <h3 className="text-2xl sm:text-3xl font-black text-white mb-2">
              Join 200+ Brokers Worldwide
            </h3>
            <p className="text-white/80 text-sm mb-6 max-w-lg mx-auto">
              Start building your forex or prop firm infrastructure today with CYTAKE.
            </p>
            <div className="flex items-center justify-center gap-4 flex-wrap">
              <motion.a
                href="/contact#demo"
                whileHover={{ scale: 1.05, y: -2 }}
                className="px-7 py-3 rounded-xl bg-white text-blue-600 font-black text-sm shadow-lg hover:shadow-xl transition-all"
              >
                Request Demo
              </motion.a>
              <motion.a
                href="/solutions"
                whileHover={{ scale: 1.05, y: -2 }}
                className="px-7 py-3 rounded-xl bg-white/15 border border-white/30 text-white font-black text-sm hover:bg-white/25 transition-all"
              >
                View Solutions →
              </motion.a>
            </div>
