  ...
  return (
    <section className="py-24 relative overflow-hidden bg-[var(--section-bg-1)]">
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        {/* Main CTA Banner — stays dark navy both modes (colored feature banner) */}
        <motion.div
        ...
        >
          ...
          {/* Inner Glowing Orbs */}
          <motion.div className="absolute top-[-20%] left-[-10%] w-[400px] h-[400px] rounded-full bg-blue-500/25 blur-[100px] pointer-events-none" animate={{ x: [0, 30, 0], y: [0, 20, 0], scale: [1, 1.2, 1] }} transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }} />
          <motion.div className="absolute bottom-[-20%] right-[-10%] w-[450px] h-[450px] rounded-full bg-sky-500/25 blur-[110px] pointer-events-none" animate={{ x: [0, -30, 0], y: [0, -20, 0], scale: [1, 1.15, 1] }} transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2 }} />
          ...
          <svg className="absolute bottom-0 left-0 w-full h-32 opacity-20 pointer-events-none" viewBox="0 0 1000 120" preserveAspectRatio="none">
            <motion.polyline points="0,90 100,70 200,80 300,50 400,60 500,35 600,45 700,25 800,40 900,15 1000,25" fill="none" stroke="#38bdf8" strokeWidth="2.5" strokeLinecap="round" initial={{ pathLength: 0 }} whileInView={{ pathLength: 1 }} viewport={{ once: true }} transition={{ duration: 2.5, ease: "easeInOut" }} />
          </svg>

          {/* Floating mini dots inside banner */}
          {[{ left: "8%", top: "20%", delay: 0 }, { left: "85%", top: "15%", delay: 1 }, { left: "50%", top: "80%", delay: 2 }, { left: "20%", top: "75%", delay: 0.5 }, { left: "75%", top: "70%", delay: 1.5 }].map((pos, i) => (
            <motion.div key={i} className="absolute w-1.5 h-1.5 rounded-full bg-sky-400 pointer-events-none" style={{ left: pos.left, top: pos.top }} animate={{ opacity: [0, 1, 0], scale: [0.5, 1.5, 0.5] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut", delay: pos.delay }} />
          ))}
          ...
          <div className="relative z-10">
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.5 }}
              ...
              <span className="text-xs font-bold text-white tracking-wide uppercase">Available Now — Launch in 7-14 Days</span>
            </motion.div>
            ...
            <motion.h2 initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }}
              ...
              <span className="bg-gradient-to-r from-sky-400 to-cyan-300 bg-clip-text text-transparent">World-Class Trading Business?</span>
            </motion.h2>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.3 }}
              ...
              {[{ val: "200+", label: "Brokers" }, { val: "50+", label: "Countries" }, { val: "7-14", label: "Days Launch" }, { val: "24/7", label: "Support" }].map((stat, i) => (
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.4 }}
              ...
              <motion.div whileHover={{ scale: 1.05, y: -2 }} className="w-full sm:w-auto">
                <Link href="/contact" className="flex items-center gap-2 justify-center rounded-xl px-8 py-4 text-base font-bold text-[#0B1B3F] bg-gradient-to-r from-white to-sky-100 shadow-[0_8px_25px_rgba(255,255,255,0.25)] hover:shadow-[0_8px_30px_rgba(56,189,248,0.4)] transition-shadow">
                  Get a Free Consultation <ArrowRight size={16} />
                </Link>
              </motion.div>

              <motion.div whileHover={{ scale: 1.05, y: -2 }} className="w-full sm:w-auto">
                <Link href="/contact#demo" className="flex items-center gap-2 justify-center px-8 py-4 rounded-xl text-base font-bold text-white bg-white/10 backdrop-blur-sm border border-white/30 hover:bg-white/20 transition-all">
            ...
            {/* Note */}
            <p className="text-slate-500 text-sm mt-6">No commitment required. Our team responds within 2 hours.</p>
          </div>
        </motion.div>
      </div>
    </section>
